import { z } from "zod";
import { parseMonthDay, parseShortYearDate } from "../utils/date.js";

const transactionSchema = z.object({
  date: z.string().regex(/^\d{2}\/\d{2}$/, "Transaction date must use MM/DD."),
...
const statementPayloadSchema = z
  .object({
    ...
    date: z.string().regex(/^\d{2}\/\d{2}\/\d{2}$/, "Statement date must use MM/DD/YY."),
    ...
    due_date: z.string().regex(/^\d{2}\/\d{2}\/\d{2}$/, "Due date must use MM/DD/YY."),
...
export function normalizeStatementPayload(payload) {
  ...
  return {
    ...
    statementDate: parseShortYearDate(parsed.date, "date"),
    rawDueDate: parsed.due_date,
    dueDate: parseShortYearDate(parsed.due_date, "due_date"),
    ...
    transactions: parsed.transactions.map((transaction, index) => {
      const parsedDate = parseMonthDay(transaction.date, `transactions[${index}].date`);
